...
export default function CoachDetailScreen() {
  ...
  if (!ad) return <LiveBackground overlay={0.5}><ScreenShell><Text selectable style={{ ...typography.headline, color: colors.textPrimary }}>{t("coach.unavailable")}</Text></ScreenShell></LiveBackground>;
  ...
  const reveal = async () => {
    ...
    try {
      ...
      Alert.alert(t("coach.contactError"), error instanceof Error ? error.message : t("auth.error.fallback"));
    } finally {
      setRevealing(false);
    }
  };
    ...
    <LiveBackground overlay={0.48}>
      <ScreenShell width="base">
        ...
        {contact ? <ContactPanel contact={contact} /> : ownAd ? (
          <View style={{ backgroundColor: colors.infoBg, borderColor: `${colors.info}44`, borderRadius: radii.lg, borderWidth: 1, padding: spacing.lg }}><Text style={{ ...typography.body, color: colors.textPrimary }}>{t("coach.ownAd")}</Text></View>
        ) : (
          <View style={{ gap: spacing.sm }}>
            <PrimaryButton label={revealing ? t("coach.openingContact") : t("coach.interested")} disabled={revealing || !player} icon={<Icon name="send" size={18} color={colors.textOnBrand as string} />} onPress={() => void reveal()} />
            <Text style={{ ...typography.footnote, color: colors.textTertiary, textAlign: "center" }}>{t("coach.interestNotice")}</Text>
          </View>
        )}
      </ScreenShell>
    </LiveBackground>

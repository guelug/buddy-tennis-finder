...
export function CoachCheckout({ ad, onActivated }: { ad: CoachAd; onActivated: () => void }) {
  ...
  const redeem = async () => {
    ...
    try {
      ...
      Alert.alert(t("coachCheckout.cantOpen").replace("{store}", storeName), error instanceof Error ? error.message : t("coachCheckout.retry"));
    } finally {
      // Dismissing the system sheet need not produce a purchase callback.
      setProcessing(false);
    }
  };
